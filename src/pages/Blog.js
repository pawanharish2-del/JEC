...
const Blog = () => {
    ...
    const getPostLink = (post) => {
        ...
        // If NOT, use the ID (but this requires the old route to exist). 
        // Ideally, SAVE ALL POSTS AGAIN in Admin to generate slugs.
                    ...
                    {featuredPost && (
                        <article className="blog-card featured">
                            ...
                            <div className="blog-content">
                                ...
                                <Link to={getPostLink(featuredPost)}>
                                ...
                                <Link to={getPostLink(featuredPost)} className="read-more">Read Full Article <i className="fas fa-arrow-right"></i></Link>
                            </div>
                        </article>
                    )}
                        ...
                        filteredPosts.map((post) => (
                            <article className="blog-card" key={post.id}>
                                ...
                                <div className="blog-content">
                                    ...
                                    <Link to={getPostLink(post)}><h2 className="blog-title">{post.title}</h2></Link>
                                    <p className="blog-excerpt">{post.excerpt}</p>
                                    {/* FIX 4: List Read More Link */}
                                    <Link to={getPostLink(post)} className="read-more">Read Full Article <i className="fas fa-arrow-right"></i></Link>
                                </div>
                            </article>
                        ))
                    ) : (
                    ...
                    <div className="widget">
                        ...
                        <ul className="recent-list">
                            {posts.slice(0, 3).map(post => (
                                <li key={post.id}>
                                    {/* FIX 5: Sidebar Link */}
                                    <Link to={getPostLink(post)} className="recent-link">
